...
class Control {
            ...
            ns.mousePressed = function() {
                if (ns.mouseButton === ns.LEFT) {
                    ...
                    for (let component of control.components) {
                        // Toggle switch/buzzer/toggle-connector/resistor.
                        // if ((component instanceof Circuit.Switch ||
                        //         component instanceof Circuit.Buzzer ||
                        //         component instanceof Circuit.ToggleConnector ||
                        //         component instanceof Circuit.Resistor) && component.contains(ns.mouseX, ns.mouseY)) {
                        ...
                        if (component instanceof Circuit.Ammeter && component.contains(ns.mouseX, ns.mouseY)) {
    ...
    showDebugInfo(component) {
        if (Control.isComponent(component)) {
            ...
            document.getElementById('c-type').innerText = component.constructor.name + ' #' + component._id;
            ...
            if (component instanceof Circuit.Bulb) {
                ...
                document.getElementById('c-other').innerHTML += '<tr><th>Colour</th><td style=\'background-color: ' + rgb + '\'>' + rgb + '</td></tr>';
            }
            ...
            if (component instanceof Circuit.ToggleConnector) {
            ...
            document.getElementById('cr-broken').innerHTML = '<span style=\'text-transform: capitalize; color: ' + (circuit._isBroken ? 'crimson' : 'green') + '\'>' + circuit._isBroken + '</span>';
    ...
    start(fps = null) {
        if (!this._running) {
            fps = (typeof fps === 'number') ? clamp(fps, 0, 60) : this._fps;
            this._running = true;
            this._p5.frameRate(fps);
            this._p5.loop();
            this._fps = fps;
        } else {
    ...
    eval() {
        ...
        if (!(this._head instanceof Circuit.Battery) && !(this._head instanceof Circuit.Cell)) {
            ...
            throw new TypeError(`_head component must be a power source`);
        }
