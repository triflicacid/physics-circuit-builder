[Circuit.Ammeter, Circuit.Voltmeter, Circuit.Wattmeter, Circuit.Lightmeter, Circuit.Thermometer] = (function() {
    /**
    ...
    class Lightmeter extends Circuit.Component {
        ...
        render() {
            super.render((p, colour, running) => {
                ...
                if (running) {
                    ...
                    if (isOn) {
                        if (this._units === 0) lumens *= 1e6; // µlm
                        else if (this._units === 1) lumens *= 1e3; // mlm
        ...
        onScroll(event) {
            ...
            if (this._units === Lightmeter.UNITS.length) this._units = 0;
            else if (this._units === -1) this._units = Lightmeter.UNITS.length - 1;
        }
    }
    ...
    class Thermometer extends Circuit.Component {
        ...
        eval() {
            super.eval(() => {
                if (this.control._running && !this._circuit._isBroken && this._externalTemp > this._max) {
                    super.blow(`Component ${this.toString()} blew as it exceeded ${this._max}°C (was ${this._externalTemp}°C)`);
                }
            });
        }
            ...
            super.render((p, colour, running) => {
                ...
                if (running && !this._blown) {
                    ...
                    if (this._externalTemp < this._min) {
                        ...
                        p.fill(217, 58, 28, a);
                        h = mapNumber(this.howFull(), 0, 1, 1, this._h - pad * 2);
                    }
                ...
                if (running) {
                    ...
                    if (this._blown) {
                        ...
                        if (temp < this._min) {
                            ...
                            if (this._units === 0) temp *= 1e6; // µ
                            else if (this._units === 1) temp *= 1e3; // m
         ...
         */
        howFull() {
            const fract = (this._externalTemp - this._min) / (this._max - this._min);
            return fract;
        }
