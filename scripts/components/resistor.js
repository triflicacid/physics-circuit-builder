[Circuit.Resistor, Circuit.VariableResistor, Circuit.LightDependantResistor, Circuit.Thermistor] = (function() {
    /**
        ...
        render(fn) {
            ...
            super.render((p, colour, running) => {
                ...
                if (this._american) {
                    ...
                    for (let i = 0, x = startX, y = 0; x < limX; x += dx, i++, y = (i % 2 === 0) ? -dy : dy) {
                        p.vertex(x, y);
                    }
                ...
                if (running && control._showInfo) {
                    ...
                    if (isOn) {
                        if (res === 0 || res === Circuit.ZERO_RESISTANCE) {
                            ...
                            else if (res < 0.1) res = roundTo(res * 1e3, 1) + 'm';
                            else if (res > 1e4) res = roundTo(res / 1e3, 1) + 'k';
     ...
     * @extends Resistor
     *
     * @method getResistance()      Calculate resistance, given this._lightRecieving
     * @method eval()
     * @method render()             Render the cell onto the global p5 sketch
     */
    class LightDependantResistor extends Resistor {
        constructor(parentCircuit) {
            super(parentCircuit);
        }
        ...
        render() {
            super.render((p, colour) => {
                ...
                const rot_angle = (this._angle + Degrees._270 - Degrees._45) + Degrees._10;
                const pad = 2.5;
     ...
     * @extends Resistor
     *
     * @property _externalHeat      Heat we are exposed to from the surroundings
     * @property _mode              Thermistor mode
     * @property _min               Minimum temperature
     * @property _max               Maximum temperature
     *
     * @method getResistance()      Calculate resistance, given this._heatRecieving
    ...
    class Thermistor extends Resistor {
        constructor(parentCircuit) {
            super(parentCircuit);
            this._externalHeat = this.control._temperature;
         ...
         */
        getResistance() {
            const temp = clamp(this._externalTemp, Control.MIN_TEMP, Control.MAX_TEMP);
            ...
            const r = mapNumber(temp, Control.MIN_TEMP, Control.MAX_TEMP, min, max);
            return r;
        }
        ...
        mode(mode) {
            if (mode === Thermistor.NTC) {
                ...
                throw new TypeError(`Thermistor.mode: invalid enum argument '${mode}'`);
            }
        }
    ...
    return [Resistor, VariableResistor, LightDependantResistor, Thermistor];
})();

Circuit.LDR = Circuit.LightDependantResistor;
Circuit.PhotoResistor = Circuit.LightDependantResistor;

Circuit.HeatDependantResistor = Circuit.Thermistor;
