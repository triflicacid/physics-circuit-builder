Circuit.Component = (function() {
    ...
    class Component {
        ...
        render(fn) {
            ...
            if (this.control._mode === Control.LIGHT && Control.isLuminous(this) && this.isOn()) {
                // DRAWING LIGHT SPHERE WAS SORTED OUT BEFOREHAND
            } else if (this.control._mode === Control.HEAT && Control.isHeatEmitter(this) && this._joules !== 0) {
                ...
                if (typeof fn === 'function') {
                    ...
                    if (this.control._mode !== Control.NORMAL) colour = p.color(150);
                    if (isBlown) colour = p.color(255, randomInt(50, 200), 0);
        ...
        getData() {
            let data = {
                ...
                pos: [this._x, this._y],
                data: {
                    maxCurrent: isFinite(this._maxCurrent) ? this._maxCurrent : Number.MAX_SAFE_INTEGER // Avoid any 'Infinity' values
                }
            };

            // If these aren't the same, they are being masked therefore no need to set
            if (this.resistance === this._resistance) data.data.resistance = this.resistance;
            ...
            if (Array.isArray(toStore) && toStore.length !== 0) {
                for (let store of toStore) {
                    ...
                    if (val === undefined) throw new TypeError(`getData(#${this._id}): cannot store undefined value '${store}' (in constructor ${this.constructor.name})`);
                    data.data[store] = val;
                }
            }
        ...
        blow(msg = undefined) {
            if (typeof msg !== 'string') {
                msg = `Component type ${this.toString()} blew on ${this.current}A, exceeding its limit of ${this.maxCurrent}A`;
            }
        ...
        trace(component, isRestrained = true, depth = 0, scannedWires = []) {
                ...
                if (wire instanceof Circuit.Wire) {
                    // Should we skip?
                    {
                        ...
                        if (this instanceof Circuit.TwoWaySwitch) {
                            skip = skip || (this._exec === Circuit.Connector.EXEC_ONE && wire[wirePropName]._circuit === this._circuit2);
                            skip = skip || (this._exec === Circuit.Connector.EXEC_TWO && wire[wirePropName]._circuit === this._circuit1);
                        }
                    ...
                    let result = wire[wirePropName].trace(component, isRestrained, depth + 1, [...scannedWires]);
            ...
            return (Array.isArray(shortest_path)) ?
                ((depth === 0) ? shortest_path : [this, ...shortest_path]) :
                null;
        }
         ...
         * @param  {Boolean} isRestrained Is the trace restrained to only go forward (true), or can we go backwards as well (false)?
         * @return {Component[] | number | String[]} array of components passed through / total thing / names of all components
         */
        roundTrip(val = 'all', isRestrained = true) {
            const trace = this.trace(this, isRestrained);
         ...
         */
        accessPath(component, isRestrained = true) {
            const trace = this.trace(this, isRestrained);
            if (trace === null) return null;
            return trace;
