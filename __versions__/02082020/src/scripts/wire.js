Circuit.Wire = (function() {
    ...
    class Wire {
         ...
         * @type {String}
         */
        get material() { return Wire.MATERIALS_KEYS[this._material]; }
        ...
        get length() {
            ...
            if (this._path.length === 0) {
                length = distance(...this._input.getOutputCoords(), ...this._output.getInputCoords());
            } else {
                const path = [
        ...
        contains(x, y) {
            ...
            let on = false;
            if (this._path.length === 0) {
                on = isNearLine(this.x1, this.y1, this.x2, this.y2, x, y, delta);
                // console.log("ONE: ", on);
            } else {
                const p = [
                    [this.x1, this.y1],
                ...
                for (let i = 0; i <= p.length - 2; i++) {
                    if (isNearLine(...p[i], ...p[i + 1], x, y, delta)) {
                        on = true;
                        break;
                    }
                }
        ...
        radiusPx(px = undefined) {
            if (typeof px === 'number') {
                this._r = clamp(px, Wire.MIN_RADIUS, Wire.MAX_RADIUS);
            }
            return this._r;
        }
        ...
        insertComponent(component) {
            ...
            if (this.contains(...pos)) {
                ...
                const delta = (this._hasResistance) ? (this._r / 2) : Wire.DEFAULT_RADIUS;
                for (let i = 0; i <= p.length - 2; i++) {
                    if (isNearLine(...p[i], ...p[i + 1], ...pos, delta)) {
    ...
    Wire.MATERIALS = {
        ...
        aluminium: { resistance: 2.82e-8, colour: [132, 135, 137] },
        antimony: { resistance: 3.90e-7, colour: [76, 79, 77] },
        bismuth: {
            resistance: 1.30e-6,
            colour: [
                [154, 155, 166],
        ...
        brass: { resistance: 0.75e-7, colour: [181, 166, 66] },
        cadmium: { resistance: 6e-8, colour: [124, 124, 128] },
        carbon_steel: { resistance: 1010, colour: [95, 96, 98] },
        cobalt: { resistance: 5.60e-8, colour: [0, 71, 171] },
        constantan: { resistance: 4.9e-7, colour: [214, 216, 219] },
        copper: { resistance: 1.68e-8, colour: [184, 115, 51] },
        germanium: { resistance: 4.60e-1, colour: [193, 194, 198] },
        gold: { resistance: 2.44e-8, colour: [212, 175, 55] },
        iron: { resistance: 1.0e-7, colour: [203, 205, 205] },
        lead: { resistance: 2.2e-7, colour: [76, 76, 76] },
        manganin: { resistance: 4.20e-7, colour: [142, 74, 43] },
        nichrome: { resistance: 1.10e-6, colour: [182, 180, 186] },
        nickel: { resistance: 6.99e-8, colour: [181, 182, 181] },
        palladium: { resistance: 1e-7, colour: [195, 198, 192] },
        platinum: { resistance: 1.06e-7, colour: [229, 228, 226] },
        silicon: { resistance: 6.40e2, colour: [68, 68, 68] },
        silver: { resistance: 1.56e-8, colour: [192, 192, 192] },
        stainless_steel: { resistance: 6.9e-7, colour: [67, 70, 75] },
        tantalum: { resistance: 1.30e-7, colour: [77, 77, 80] },
        tin: { resistance: 1.09e-7, colour: [211, 212, 213] },
        titanium: { resistance: 4.20e-7, colour: [135, 134, 129] },
        tungsten: { resistance: 5.60e-8, colour: [118, 121, 128] },
        zinc: { resistance: 5.90e-8, colour: [186, 196, 200] },
    };
