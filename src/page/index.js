...
const Page = {
    ...
    controls: {
        ...
        init() {
            ...
            for (let link of links) {
                ...
                img.src = "src/images/" + utils.toClassName(component) + ".png";
        ...
        prep() {
            ...
            utils.eventOn(Page.controls.lightSlider, "input");
            ...
            utils.eventOn(Page.controls.temperatureSlider, "input");
        ...
        analyse(c) {
            ...
            if (Control.isComponent(c)) {
                ...
                info.cMaxCurrent.innerHTML = !isFinite(c.maxCurrent) ||
                    ...
                    "&infin;" :
                    utils.numberFormat(c.maxCurrent, 3);
                info.cIsOn.innerHTML = utils.getHtmlBoolString(c.isOn());
                info.cIsBlown.innerHTML = utils.getHtmlBoolString(c.isBlown());
                info.cExternLight.innerHTML = utils.roundTo(c._lightRecieving, 2);
                info.cExternTemp.innerHTML = utils.roundTo(c._externalTemp, 2);
                ...
                if (typeof c._lpw === "number")
                    other.push([
                        "Luminoscity",
                        utils.roundTo(c.luminoscity(), 2) + "lm @ " + c._lpw + "lm/w",
                    ]);
                ...
                if (c instanceof Circuit.Bulb) {
                    ...
                    other.push(
                        ...
                        ["Old Symbol", utils.getHtmlBoolString(c._old)]
                    );
                }
                ...
                if (c instanceof Circuit.Resistor)
                    other.push([
                        ...
                        utils.getHtmlBoolString(c._american),
                    ]);
                ...
                if (c instanceof Circuit.Buzzer)
                    other.push(
                        ["Volume", c.volume() * 100 + "%"],
                        ["Mute", utils.getHtmlBoolString(c.mute())],
                        ["Freq", c.frequency() + "Hz"]
                    );
                ...
                if (c instanceof Circuit.Motor)
                    other.push(
                        [
                            ...
                            utils.roundTo(rad2deg(c.K), 1) + "&deg;",
                        ],
                        ["Speed", utils.roundTo(rad2deg(c.delta()), 2) + "&deg; / frame"],
                        ["Angle", c.angle()]
                    );
                ...
                if (c instanceof Circuit.MaterialContainer)
                    other.push(
                        ["Material", utils.nicifyString(c.material, ' ')],
                        ["Length", utils.roundTo(c._w / Page.control.PIXELS_PER_CM, 3) + 'cm (' + c._w + 'px)'],
                        ["Volume", utils.numberFormat(c.volume(true), 3) + 'cm³ (' + Math.round(c.volume(false)) + 'px)'],
                    );
                if (c instanceof Circuit.WireContainer)
                    other.push(
                        ["Material", utils.nicifyString(c.material, ' ')],
                        ["Length", utils.roundTo(c.length(true), 3) + 'cm (' + c.length(false) + 'px)'],
                        ["Volume", utils.numberFormat(c.volume(true), 3) + 'cm³ (' + Math.round(c.volume(false)) + 'px)'],
                    );
                ...
                info.wLength.innerText = `${utils.roundTo(length / Page.control.PIXELS_PER_CM, 2)}cm (${Math.round(length)}px)`;
                ...
                if (c._hasResistance) {
                    ...
                    let html = `<input type='range' min='${Circuit.Wire.MIN_RADIUS}' value='${c.radiusPx()}' step='0.1' max='${Circuit.Wire.MAX_RADIUS}' oninput='${onclick}' onchange='Page.controls.analyse(1);' /> <span id='analyse-w-radius-text'>${utils.roundTo(c.radiusCm(), 3)}cm (${utils.roundTo(c.radiusPx(), 1)}px)</span>`;
                    info.wRadius.innerHTML = html;
                } else {
                    info.wRadius.innerText = utils.roundTo(1.5 / Page.control.PIXELS_PER_CM, 3) + 'cm';
                }
                ...
                for (let i = 0; i < Circuit.Wire.MATERIALS_KEYS.length; i++) {
                    ...
                    html += `<option value="${i}"${c._material === i ? " selected" : ""}>${utils.nicifyString(material, ' ')} (${utils.numberFormat(Circuit.Wire.MATERIALS[material].resistance, 2) } Ω/m³)</option>`;
                }
     ...
     */
    circuitError(title, ...msg) {
        return new Popup(title, ...msg)
            .mode("error")
            .onClose(() => Page.controls.stop())
            .open();
    },
};
