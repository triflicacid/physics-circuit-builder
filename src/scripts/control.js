...
class Control {
    ...
    load(data, callback = undefined) {
        ...
        const __draw__ = function(ns, control) {
            ...
            document.getElementById("lastUpdate").innerText = `${date
                ...
                .getMilliseconds()
                .toString()
                .padStart(3, "0")}; Frames: ${ns.frameCount} (${roundTo(
                ns.frameRate(),
                1
            )} fps)`;
                            ...
                            if (
                                ...
                                if (control._isCreatingWire) {
                                    try {
                                        ...
                                        new Popup("Cannot Connect Components")
                                            .mode("error")
                                            .msg(e)
                                            .onClose(() => Page.controls.stop())
                                            .open();
                ...
                if (ns.key === "Delete" || ns.key === 'Backspace') {
                    ...
                    // If over wire...
                        ...
                        if (Array.isArray(handle) && window.confirm(`Delete wire handle at (${Math.round(handle[0])}, ${Math.round(handle[1])}) ?`)) {
                            arrRemove(handle, control._over._path);
    ...
    createComponent(rawName, x, y) {
        ...
        if (typeof klass !== "function")
            throw new ComponentError(
                `createComponent: component Circuit.${name} does not exist`
            );
    ...
    render() {
        ...
        // Show ambient light
        {
            const roundness = 20;
            const h = 10;
            const ambient = clamp(this._lightLevel, 0, 1000);
            if (ambient !== this._lastLightLevel) {
                const brightness = ambient / 10;
                this._lightLevelRgb = hsb2rgb(60, 100, brightness);
        ...
        if (this._mode === Control.LIGHT) {
            ...
            this.components.forEach((c) => {
                if (Control.isLuminous(c) && c.isOn()) {
                    ...
                    const end = p.color(
                        ...hsb2rgb(p.hue(start), 100, c.control._lightLevel)
                    );
