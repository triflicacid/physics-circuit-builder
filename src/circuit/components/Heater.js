...
class Heater extends Component {
    ...
    celcius(val = undefined) {
        if (typeof val === "number") {
            this._joules = utils.clamp(utils.deg2joules(val), 0, this._maxTemp);
            return val;
        } else {
            return utils.joules2deg(this._joules);
        }
    }
    ...
    maxTemp(val = undefined) {
        if (typeof val === "number") {
            this._maxTemp = utils.clamp(val, 0, Control.MAX_TEMP);
            this._maxJoules = utils.deg2joules(this._maxTemp);
            return val;
        } else {
    ...
    temp(dp = 3) {
        ...
        temp = utils.roundTo(temp, dp).toFixed(dp);
        return temp + "°C";
    }
